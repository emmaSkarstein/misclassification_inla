...
n_runs <- 5 # No. of simulated data sets
niter <- 200000 # No. of iterations for importance sampling
...
all_runs$Model <- plyr::revalue(all_runs$Model, 
                               c("naive" = "Model fit on only complete observations", 
                                 "inla_is" = "Model with imputation", 
                                 "correct" = "Model using correct value for missing observations"))
...
ggplot(all_runs, aes(x = mean, y = iteration, color = Model)) +
  ...
  theme_bw() 
ggsave(paste0("figures/missing_simulated_", name_append, ".pdf"))
```
